...
#define _W2A(lpw) ( (lpw == NULL ) ? NULL : _W2AHelp(lpw, (LPSTR) _malloca( wcslen( lpw ) + 1 ) ) )
...
bool g_bIsWinNT = !IsWindowsXPOrGreater(); // (GetVersion() < 0x80000000);
		...
		)
{
	if( pszAdd ) {
		...
		if( n <= 1024 ) {
			...
			LPWSTR pwsz = (LPWSTR)_malloca( n * 2 );
		...
		)
{
	...
	if( nStartChar == nSize && nSize == nEndChar  ) {
		...
		if( si.nPos >= si.nMax - (int) si.nPage + 1  ) {
			...
			{
				LPSTR p = _W2A(wsText.c_str());
				::SendMessage(hEdit, EM_REPLACESEL, 0, (LPARAM)p);
				if (p != nullptr)
					_freea(p);
			}
			bAdded = true;
