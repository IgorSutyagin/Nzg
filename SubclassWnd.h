...
#error Only Alpha and X86 supported
		...
		WndProcThunk &operator=(const WndProcThunk &); // prevent copy
		WndProcThunk(const WndProcThunk &); // prevent copy
	public:

		WndProcThunk()
		{
			m_pThunk = static_cast<Thunk*>(VirtualAlloc(NULL, sizeof(*m_pThunk), MEM_COMMIT | MEM_RESERVE, PAGE_READONLY));
			ASSERT(m_pThunk);
		}
		~WndProcThunk()
		{
			if (m_pThunk)
				VirtualFree(static_cast<LPVOID>(m_pThunk), NULL, MEM_RELEASE);
			m_pThunk = 0;
		}
		WNDPROC GetWndProc()
		{
		...
		{
			DWORD flOldProtect;
			VirtualProtect(static_cast<LPVOID>(m_pThunk), sizeof(*m_pThunk), PAGE_READWRITE, &flOldProtect);
			Thunk &thunk = *m_pThunk;
#if defined (_M_IX86)
			thunk.m_mov = 0x042444C7;  //C7 44 24 0C
			thunk.m_this = (DWORD)pThis;
			thunk.m_jmp = 0xe9;
			thunk.m_relproc = (int)proc - ((int)&thunk.m_relproc + sizeof(thunk.m_relproc));
#elif defined (_M_ALPHA)
...
#endif
			VirtualProtect(static_cast<LPVOID>(m_pThunk), sizeof(*m_pThunk), PAGE_EXECUTE_READ, &flOldProtect);
